...
描述：
	属性修正 Resource
	定义单个属性的加成，支持加法/乘法/百分比等运算

用法：
	1. 在编辑器中新建 Resource，类型选 StatModifier
	2. 填写 stat_name / operation / value
	3. 配合 StatComponent 或自定义属性系统使用

	# 代码示例：
	var mod = StatModifier.new()
	mod.stat_name = "attack"
	mod.operation = StatModifier.Op.MULTIPLY
	mod.value = 1.5  # +50% 攻击力
'''
