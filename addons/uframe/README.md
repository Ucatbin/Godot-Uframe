...
- `Maximum Size > 0`：限制池拥有的总实例数，预热也不会超过它；
- `Overflow Policy = 达到上限返回空值`：默认且最安全，满载时 `acquire()` 返回 `null`；
- `Overflow Policy = 复用最早活跃实例`：执行旧实例的 release 生命周期后立即复用同一 Node，不会 `queue_free()` 再创建。

“最早”指当前仍在使用的实例中，最近一次成功 `acquire()` 最早者；实例被归还后再次取出会成为最新。强制复用适合粒子、拖尾、弹孔等允许提前消失的表现。敌人、任务物体或带有旧 `call_deferred()` 归还回调的对象应保留默认策略，因为旧持有者仍引用同一个 Node，延迟回调可能干扰它的新生命周期。强制复用完成后会额外发送 `instance_recycled`。
...
- `UFrameStateMachine` + `UFrameState`：唯一节点式状态机；直接挂在实体下，状态作为它的子节点。状态机自动转发普通帧与物理帧更新，每个状态只需覆写需要的回调。平台示例的 Idle、Run、Air 分别真正执行移动、跳跃和重力逻辑，而不是空标签。
...
- 对象池的计数查询为 O(1)，普通空闲复用不扫描全池；只有真正触顶时才清理失效引用并执行可选溢出策略。
- 对象池不会自动猜测业务状态，实例自行实现池生命周期钩子；最早实例回收默认关闭。
