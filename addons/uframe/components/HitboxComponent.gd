...
#region 信号
## 目标实际扣除生命后发出。
## [param target] 是受到命中的节点，[param applied_damage] 是实际造成的伤害。
...
## 当本组件是池化场景根节点时，由 [UFramePool] 取出实例时调用。
...
#region 命中生命周期
## 记录一次已经生效的命中并发出 [signal hit_confirmed]。
## 仅应由伤害结算方在 [param target] 实际扣血后调用；[param applied_damage] 是实际伤害。
...
#region 命中查询
## 判断 [param target] 是否可以在本次激活中被命中。
## 目标无效，或启用单次限制且已经命中过时返回 [code]false[/code]。
