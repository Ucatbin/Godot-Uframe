...
#region 生命周期
## 插件实例进入编辑器时注册设置的显示信息，并补充新版新增的缺失设置。
...
#region 内部方法
## 补充缺失设置，并为 Project Settings 注册布尔类型和基础设置标记。
...
## 仅移除仍然指向插件运行时脚本的 UFrame Autoload。
...
## 返回当前 UFrame Autoload 的规范化资源路径；不存在时返回空字符串。
