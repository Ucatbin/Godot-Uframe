...
#region 查询方法
## 判断动作是否已缓冲；输入锁定时始终返回 [code]false[/code]。
func is_action_buffered(action: String) -> bool:
	if _input_locked:
		return false
	return _buffer.get(action, 0.0) > 0.0

## 查询动作是否刚按下；输入锁定时返回 [code]false[/code]。
func is_action_just_pressed(action: String) -> bool:
	if _input_locked:
		return false
	return Input.is_action_just_pressed(action)

## 查询动作是否按住；输入锁定时返回 [code]false[/code]。
func is_action_pressed(action: String) -> bool:
	if _input_locked:
		return false
	return Input.is_action_pressed(action)

## 查询动作是否刚释放；输入锁定时返回 [code]false[/code]。
func is_action_just_released(action: String) -> bool:
	if _input_locked:
		return false
	return Input.is_action_just_released(action)

## 获取输入方向；输入锁定时返回 [constant Vector2.ZERO]。
## [param deadzone] 为负数时使用项目设置中的动作死区。
